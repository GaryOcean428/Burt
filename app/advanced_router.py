...
class AdvancedRouter:
    ...
    def classify_task(self, user_input: str) -> str:
        if any(keyword in user_input.lower() for keyword in ["code", "program", "function", "algorithm"]):
            return "coding"
        elif any(keyword in user_input.lower() for keyword in ["creative", "story", "imagine", "art"]):
            return "creative"
        elif any(keyword in user_input.lower() for keyword in ["analyze", "research", "compare", "evaluate"]):
            return "analysis"
    ...
    def is_complex_task(self, user_input: str) -> bool:
        complex_keywords = ["analyze", "explain", "compare", "synthesize", "evaluate", "design", "optimize", "predict", "simulate"]
    ...
    def select_large_model(self, task_type: str) -> str:
        if task_type == "coding":
            ...
            return random.choice(["llama-3.1-sonar-large-128k-online", "llama-3.1-sonar-huge-128k-online"])
        else:
            return random.choice(["gpt-4o", "claude-3.5-sonnet", "llama-3.1-405b", "gemini-1.5-pro"])
