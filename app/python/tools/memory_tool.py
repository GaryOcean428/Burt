...
from app.python.helpers.mongodb_client import insert_document, find_documents, delete_document
...
def save(agent: Agent, text: str) -> str:
    ...
    document_id = insert_document("memories", {
        "content": text,
        "vector_id": vector_id
    }).inserted_id
...
def upload_file(agent: Agent, file_path: str) -> str:
    ...
    return files.read_file("./prompts/fw.file_uploaded.md", file_path=file_path, memory_id=str(document_id))
