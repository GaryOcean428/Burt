...
from groq import Groq  # Add this import at the top of the file
...
def get_lmstudio_chat(
    ...
    return ChatOpenAI(model=model_name, base_url=base_url, temperature=temperature, api_key=None)
...
def get_azure_openai_chat(
    ...
    return AzureChatOpenAI(model=deployment_name, temperature=temperature, api_key=api_key, azure_endpoint=azure_endpoint)
...
def get_azure_openai_instruct(
    ...
    api_key = api_key or get_api_key("openai_azure")
    azure_endpoint = azure_endpoint or os.getenv("OPENAI_AZURE_ENDPOINT")
    return AzureOpenAI(model=deployment_name, temperature=temperature, api_key=api_key, azure_endpoint=azure_endpoint)

def get_azure_openai_embedding(deployment_name: str, api_key=None, azure_endpoint=None):
    api_key = api_key or get_api_key("openai_azure")
    azure_endpoint = azure_endpoint or os.getenv("OPENAI_AZURE_ENDPOINT")
    return AzureOpenAIEmbeddings(model=deployment_name, api_key=api_key, azure_endpoint=azure_endpoint)
...
def get_perplexity_chat(model_name: str = "sonar-medium-chat", api_key=None, temperature=DEFAULT_TEMPERATURE):
...
def get_model(model_name: str) -> BaseChatModel | BaseLLM:
    if model_name.startswith("gpt-4"):
        return get_openai_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
    elif model_name.startswith("claude"):
        return get_anthropic_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
    elif model_name.startswith("llama-3.1"):
        if "sonar" in model_name:
            return get_perplexity_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
        else:
            return get_groq_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
    elif model_name.startswith("gemini"):
        return get_google_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
    elif model_name.startswith("mistral") or model_name.startswith("mixtral"):
        return get_mistral_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
    elif model_name in ["command-r-plus", "reka-core"]:
        return get_openrouter_chat(model_name, api_key=None, temperature=DEFAULT_TEMPERATURE)
    else:
